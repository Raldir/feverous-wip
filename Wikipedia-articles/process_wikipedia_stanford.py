...
import traceback
from stanza.server import CoreNLPClient, to_text
...
class Extractor(object):
    ...
    def parse_article(self, doc, title, tokenizer):
        ...
        for i, element in enumerate(content):
            ...
            if math_img and math_img.has_attr('class') and len([s for s in math_img.get('class') if 'math' in s]) > 0: # exclude equations
                ...
                text = text.replace('\n', ' ')
                sent = tokenizer.annotate(text).sentence
                for tok in sent:
                    tok = to_text(tok)
...
if __name__ == '__main__':
    ...
    if mode == 'clean':
        ...
        def process_file(member, nlp):
            title, content = member
            extractor = Extractor()
            #print(title)
            try:
                processed_content, processed_data = extractor.parse_article(content, title, nlp)
        ...
        nlp =  CoreNLPClient(annotators=['tokenize','ssplit'], timeout=300000,  be_quiet = True, memory='16G', threads= 8)
        ...
        for lob, file in enumerate(tqdm(file_paths)):
            ...
            # with Pool(1) as p:
            for tar in extracted_tars:
                results = process_file(tar, nlp)
