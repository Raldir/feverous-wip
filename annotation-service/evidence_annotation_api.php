...
if ($_SESSION['annotation_mode'] != 'evidence') {
  echo 'Annotator is assigned to different task';
  file_put_contents($file, "Annotator is assigned to different task" . $date.PHP_EOL, FILE_APPEND | LOCK_EX);
  return;
}
...
if ($req == "report-claim"){ //&& $finished_calibration == 1
  ...
  if($result->num_rows > 0){
    if ($row['current_task'] != 0) {
      ...
      if($choice < $prob_new && $finished_calibration!=0){
        $sql = "SELECT cl.id, cl.claim, cl.skipped, cl.annotator FROM $claim_table cl WHERE (cl.evidence_annotators_num = (SELECT MIN(cq.evidence_annotators_num) FROM $claim_table cq) AND cl.taken_flag=0  AND cl.date_made  < DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND  cl.evidence_annotators_num<=$max_annotators_per_claim AND cl.annotator != $user_id +1 AND cl.id NOT IN (SELECT ev.claim FROM $evidence_table ev WHERE (ev.annotator = $user_id AND ev.claim = cl.id))) ORDER BY RAND() LIMIT 1 ";
        ...
        $output =  array($row['id'], $row['claim']);
        if(empty($row['id'])){
          $sql = "SELECT cl.id, cl.claim, cl.skipped, cl.annotator FROM $claim_table cl WHERE  (cl.evidence_annotators_num > 0 AND cl.taken_flag=0 AND cl.date_made  < DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND cl.evidence_annotators_num<=$max_annotators_per_claim AND cl.annotator != $user_id +1 AND cl.id NOT IN (SELECT ev.claim FROM $evidence_table ev WHERE (ev.annotator = $user_id AND ev.claim = cl.id))) ORDER BY RAND() LIMIT 1";
          $result = $conn->query($sql);
      ...
      if(empty($row['id']) && $finished_calibration == 0){
        $conn->begin_transaction();
        update_table($conn, "UPDATE Annotators SET finished_calibration=1 WHERE id=?", 'i', $user_id);
        $conn->commit();
  ...
  // $user_id = 1;
  // $claim_id = 1;
  // $evidence = ['Anarchism_sentence_8', 'Anarchism_sentence_9'];
  // $evidence = implode(" [SEP] ", $evidence);
  // $verdict = 'supported';
  // $details = ['When the water falls I am happy.'];
  // $details = implode(" [SEP] ", $details);
