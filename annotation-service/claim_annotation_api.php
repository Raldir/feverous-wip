...
if ($finished_calibration == 1){
  ...
  $claim_data_table = "CalibrationClaimAnnotationData";
...
if ($req == "skip-annotation" && $finished_calibration == 1){
  ...
  if($result->num_rows > 0){
    if ($row['current_task'] != 0) {
      ...
      $sql = "SELECT id, page, is_table, selected_id, manipulation, quick_hyperlinks, multiple_pages FROM $claim_data_table WHERE id = ?";
      ...
      $result = $stmt->get_result();
      $row = $result->fetch_assoc(); //get first result
      $output =  array($row['id'], $row['page'], $row['is_table'], $row['selected_id'], $row['manipulation'], $row['quick_hyperlinks'], $row['multiple_pages']);
  ...
  if($result->num_rows > 0 && $result_c2->num_rows > 0 && $result_c3->num_rows > 0){
    ...
    $sql = "SELECT selected_id, manipulation, multiple_pages, is_table FROM $claim_data_table WHERE id = ?";
    $stmt= $conn->prepare($sql);
    ...
    echo json_encode(array($row['claim'], $row_c2['claim'], $row_c3['claim'], $row['data_source'], $data['selected_id'], $data['manipulation'], $data['multiple_pages'], $row['page'], $row_c2['search'], $row_c2['hyperlinks'], $row['search_order'], $row['total_annotation_time'], $row['annotation_time_events'], $row['challenges'], $row_c2['challenges'], $row_c3['challenges'], $data['is_table']));
  }else{
